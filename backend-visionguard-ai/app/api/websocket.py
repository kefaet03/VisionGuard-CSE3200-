...
# =========================================================================
# Owl Eye alert throttling
# =========================================================================


# Only applies to Owl Eye (sentry mode) messages.
# Keyed by (shop_id, location) so each camera/location can alert independently.
_owl_eye_last_alert_monotonic: Dict[Tuple[str, str], float] = {}
OWL_EYE_LOCATION_COOLDOWN_SECONDS = 2.0
...
async def handle_owl_eye_detection(user_id: str, message: dict, websocket: WebSocket):
    ...
    try:
        ...
        # Throttle Owl Eye alerts per (shop_id, location).
        # If the same location keeps detecting persons, only alert once every 2 seconds.
        if detections and len(detections) > 0:
            now = time.monotonic()
            throttle_key = (str(shop_id), str(location))
            last = _owl_eye_last_alert_monotonic.get(throttle_key)
            if last is not None and (now - last) < OWL_EYE_LOCATION_COOLDOWN_SECONDS:
                logger.info(
                    f"[Owl Eye] Suppressed alert for user {user_id}, shop {shop_id}, location '{location}' "
                    f"(cooldown {OWL_EYE_LOCATION_COOLDOWN_SECONDS}s)"
                )
                return
        ...
        try:
            ...
            if anomaly:
                # Mark alert time only after successful anomaly creation to ensure cooldown
                # tracks actual triggered alerts.
                if detections and len(detections) > 0:
                    _owl_eye_last_alert_monotonic[(str(shop_id), str(location))] = time.monotonic()
