...
from app.core.dependencies import get_accessible_shop
...
class DemoAnomalyNotificationRequest(BaseModel):
    stream_id: str
    person_id: int = 1
    trigger_second: int = 17
...
@router.post(
    "/demo-anomaly/{shop_id}",
    summary="Send Demo Anomaly Telegram Alert",
    description="Used for lab/demo: sends a Telegram alert for a forced anomaly trigger.",
)
async def send_demo_anomaly_telegram_alert(
    shop_id: UUID,
    payload: DemoAnomalyNotificationRequest,
    shop: Shop = Depends(get_accessible_shop),
):
    telegram_chat_id = shop.telegram_chat_id
    if not telegram_chat_id:
        raise HTTPException(status_code=400, detail="Shop is not connected to Telegram")

    when = payload.timestamp or "now"
    text = (
        f"Anomaly triggered.\n"
        f"Shop: {shop.name}\n"
        f"Stream: {payload.stream_id}\n"
        f"Person ID: {payload.person_id}\n"
        f"Trigger: {payload.trigger_second}s\n"
        f"Time: {when}"
    )

    telegram_service = get_telegram_service()
    ok = await telegram_service.send_notification(
